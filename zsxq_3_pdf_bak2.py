...
class zsxq_work(object):
    ...
    def get_zsxq_essence_content_pdf(self, group_id, group_name):
        """
        因为内容太多，我只想看精华帖子
        :return:
        """
        while True:
            url_content = 'https://api.zsxq.com/v1.10/groups/{}/topics?scope=digests&count=20&end_time={}'.format(
                ...
                self.end_time)
            print(url_content)
            response = requests.get(url=url_content, headers=self.zsxq_headers)  # 一定要加headers，规范写法，就像过马路一样穿红灯有时没事，有时要命！
            if response.status_code == 200:  # 注意：这里一定要做200判断，
                topics = json.loads(response.text, encoding="utf-8").get("resp_data").get(
                    "topics")  # 把unicode 编码成 utf-8
                print(topics)
                if not topics:  # 如果没有主题就退出
                    print('没有数据了,开始制作pdf文档')
...
if __name__ == '__main__':
    xq = zsxq_work()
    xq.get_zsxq_group()
    xq.get_zsxq_essence_content_pdf(xq.zsxq_group_id[0], xq.zsxq_group_name[0])
    # for i in range(len(xq.zsxq_group_id)):
    #     xq.end_time = 0  # 每换一个星球群组，需要把时间戳重置下
    #     xq.get_zsxq_essence_content_pdf(xq.zsxq_group_id[i], xq.zsxq_group_name[i])
